...
export default function SexualOrientationInfo({ navigation }) {
  const { sexualOrientation, setSexualOrientation } = useInfo(); // Sexual orientation that user will add to an array to represent themselves
  const [buttonStyleList, setButtonStyleList] = useState(
    new Array(8).fill(styles.orientationViews)
  );
  const [clickedButtonList, setClickedButtonList] = useState(
    new Array(8).fill(false)
  ); // Determines whether a button was clicked or not
  const sexualOrientationsList = [
    "Gay",
    "Lesbian",
    "Bisexual",
    "Pansexual",
    ...
    "Questioning",
    "Demisexual",
  ]; // Array of sexual orientations to choose from
  ...
  const handleOrientation = (position) => {
    ...
    const newButtonStyleList = buttonStyleList.map((item, index) => {
      if (position === index) {
        if (buttonStyleList[position] === styles.orientationViews) {
          setSexualOrientation(sexualOrientationsList[position]);
          return styles.clickedOrientationViews;
        } else {
          return styles.orientationViews;
        }
      } else {
        return styles.orientationViews;
      }
  ...
  useFocusEffect(
    React.useCallback(() => {
      const selectedTypeIndex =
        sexualOrientationsList.indexOf(sexualOrientation);
      const newButtonStyleList = buttonStyleList.map((item, index) =>
        selectedTypeIndex === index
          ? styles.clickedOrientationViews
          : styles.orientationViews
      );

      setClickedButtonList((prev) =>
        newButtonStyleList.map(
          (style, index) => style === styles.clickedOrientationViews
        )
      );
  ...
  return (
    ...
    >
      <View style={styles.signinbox}>
        ...
        <View style={styles.innerOrientationSectionView}>
          {sexualOrientationsList.map((item, index) => {
            return (
              <Pressable
                style={buttonStyleList[index]}
                onPress={() => handleOrientation(index)}
                key={index}
              >
